...
const Icon = ({ name, className = "", style }: { name: string; className?: string; style?: React.CSSProperties }) => {
  ...
  if (name === 'phone') {
    return (
      <svg className={`w-5 h-5 ${className}`} fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24" style={style || { filter: 'brightness(0) opacity(0.7)' }}>
  ...
  if (name === 'slack') {
    return (
      <svg className={`w-5 h-5 ${className}`} fill="currentColor" viewBox="0 0 24 24" style={style || { filter: 'brightness(0) opacity(0.7)' }}>
