          ...
          {changelogData.map((entry, index) => (
            <div key={index} className="grid md:grid-cols-[200px_1fr] gap-8 items-start">
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">
                ...
                <Badge variant="outline" className="mt-2 text-blue-700 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 border-blue-200 dark:border-blue-500/50 font-medium">{entry.category}</Badge>
              </div>

              {/* Title and Description */}
              <div>
                <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-3">{entry.title}</h2>
                <p className="text-gray-600 dark:text-gray-400 leading-relaxed">{entry.description}</p>
              </div>
            </div>
          ))}
        </div>
