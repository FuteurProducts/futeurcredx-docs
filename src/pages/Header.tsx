      ...
      <div className={`sticky top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <header className="flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
          ...
          <div className="flex items-center">
            <SmartLink to="/" className="cursor-pointer">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            <SmartLink to="/" className={`transition-colors ${
              isScrolled 
                ? isTransparentHeader 
            ...
            <SmartLink to="/business" className={`transition-colors ${
              isScrolled 
                ? isTransparentHeader 
                  ? useBlackText ? 'text-slate-700 hover:text-blue-600' : 'text-white hover:text-gray-300'
                  : 'text-white hover:text-gray-300'
            ...
            <SmartLink to="/lumiq-build" className={`transition-colors ${
              isScrolled 
                ? isTransparentHeader 
                  ? useBlackText ? 'text-slate-700 hover:text-blue-600' : 'text-white hover:text-gray-300'
                  : 'text-white hover:text-gray-300'
            ...
            <SmartLink to="/credit-journey" className={`transition-colors ${
              isScrolled 
                ? isTransparentHeader 
                  ? useBlackText ? 'text-slate-700 hover:text-blue-600' : 'text-white hover:text-gray-300'
                  : 'text-white hover:text-gray-300'
          ...
          <div className="flex flex-col space-y-6 p-6">
            ...
            <div className="flex flex-col space-y-4 pt-4">
              <div onClick={() => setMenuOpen(false)}>
                <div style={{
                  '--custom-mobile-bg-color': isWhiteHeader ? '#3B82F6' : isScrolled && useBlackText ? '#07124A' : '#07124A'
                } as React.CSSProperties}>
                  <QRCodeModal 
                    buttonText="Download the App" 
                    buttonClassName={`rounded-lg py-5 text-xl font-bold w-full transition-colors ${
                      isWhiteHeader
                        ? 'text-white hover:opacity-80'
                        : isTransparentHeader
                          ? isScrolled && useBlackText ? 'text-white hover:opacity-80' : 'text-white hover:opacity-80'
                          : 'bg-white text-black hover:bg-gray-100'
                    } ${isWhiteHeader || isTransparentHeader ? '[background-color:var(--custom-mobile-bg-color)]' : ''}`}
                  />
                </div>
              </div>
            </div>
