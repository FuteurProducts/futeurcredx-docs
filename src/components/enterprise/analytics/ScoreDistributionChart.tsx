      ...
      <div className="bg-white border border-border rounded-xl p-4 shadow-lg">
        ...
        <div className="space-y-1 text-sm">
          ...
          <div className="flex justify-between gap-4">
            ...
            <span className="font-semibold">{data.percent.toFixed(1)}%</span>
          </div>
          <div className="flex justify-between gap-4">
            <span className="text-muted-foreground">Exposure:</span>
            <span className="font-semibold">${(data.exposure / 1000000).toFixed(1)}M</span>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative group">
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-foreground text-background text-xs rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50">
              Distribution of LumiqAI scores across portfolio
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
            ...
            <Bar dataKey="percent" radius={[6, 6, 0, 0]} maxBarSize={80}>
              {data.map((entry) => (
                <Cell key={entry.range} fill={bucketColors[entry.range] || '#6b7280'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      ...
      <div className="flex flex-wrap justify-center gap-4 mt-4 pt-4 border-t border-border">
        {data.map((bucket) => (
          <div key={bucket.range} className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold">{bucket.percent}%</span>
          </div>
        ))}
      </div>
    </motion.div>
