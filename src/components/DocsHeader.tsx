...
export default function DocsHeader({ searchQuery, setSearchQuery }: DocsHeaderProps) {
  ...
  return (
        <div className="fixed top-0 left-0 right-0 z-50 bg-white dark:bg-black border-b border-gray-200 dark:border-white/10">
            <header className="relative flex items-center justify-between px-6 py-4 max-w-7xl mx-auto">
                <div className="absolute bottom-0 left-6 right-6 h-px bg-gray-200 dark:bg-white/10" />
        ...
        {/* Search Bar */}
          ...
          <div className="relative w-full max-w-md">
            ...
            <input
              ref={searchInputRef}
                            className="w-full h-9 pl-10 pr-20 rounded-md text-sm transition-all duration-200 bg-gray-100 dark:bg-black border border-gray-300 dark:border-white/80 text-black dark:text-white placeholder-gray-500 dark:placeholder-white focus:outline-none focus:ring-1 focus:ring-blue-500 dark:focus:ring-white/50 border-transparent focus:border-blue-500 dark:focus:border-white/50"
              type="text"
              placeholder="Search or ask a question..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
                                    <kbd className="absolute top-1/2 right-3 -translate-y-1/2 hidden h-5 select-none items-center gap-1 rounded border bg-gray-100 dark:bg-black border-gray-300 dark:border-gray-600 px-1.5 font-mono text-xs text-gray-600 dark:text-gray-400 sm:flex">
              ⌘K
            </kbd>
          </div>
        </div>
