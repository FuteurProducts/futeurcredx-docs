    ...
    return (
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-slate-700">Approval Readiness</h3>
          ...
          {stages.map((stage, index) => (
            <React.Fragment key={stage.id}>
              <div className="flex flex-col items-center">
                ...
                <span className="text-[10px] text-slate-500 mt-1 font-medium">{stage.name}</span>
              </div>
        ...
        {data.nextUnlock && (
          <div className="flex items-center justify-between text-sm pt-3 border-t border-slate-100">
            <span className="text-slate-500">Next unlock</span>
            <span className="font-semibold text-slate-900">{data.nextUnlock}</span>
          </div>
        )}
      </motion.div>
    );
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 mb-6">
          ...
          <h3 className="text-lg font-semibold text-slate-900">Credit Journey</h3>
        </div>
            ...
            <div key={stage.id} className="relative">
              ...
              <div className={`flex gap-4 p-3 rounded-xl transition-colors ${
                ...
                <div className="flex-1">
                  ...
                  <p className={`text-sm ${
                    stage.status === 'locked' ? 'text-slate-400' : 'text-slate-500'
                  }`}>
                    {stage.description}
                  </p>
        ...
        {data.estimatedReadiness && (
          <div className="mt-6 p-4 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl">
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-600">Est. readiness</span>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-purple-500" />
          <h3 className="text-lg font-semibold text-slate-900">Approval Readiness</h3>
        </div>
        {data.estimatedReadiness && (
        ...
        {stages.map((stage, index) => (
          <div key={stage.id} className="flex flex-col items-center z-10">
            ...
            <span className={`text-sm font-medium mt-2 ${
              stage.status === 'locked' ? 'text-slate-400' : 'text-slate-700'
            }`}>
              {stage.name}
            </span>
          </div>
      ...
      {stages[data.currentStage] && (
        <div className="bg-blue-50 rounded-xl p-4">
          ...
          <h4 className="font-semibold text-slate-900 mb-1">{stages[data.currentStage].name}</h4>
          <p className="text-sm text-slate-600 mb-3">{stages[data.currentStage].description}</p>
      ...
      {data.nextUnlock && (
        <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
          <span className="text-sm text-slate-500">Next unlock</span>
          <span className="font-semibold text-slate-900">{data.nextUnlock}</span>
        </div>
      )}
    </motion.div>
